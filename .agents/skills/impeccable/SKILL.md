...
1. Load PRODUCT.md, DESIGN.md, and the matching surface brief when present; follow their directives. Pass a named source file or route as `--target <path>`.
...
After init writes PRODUCT.md, resume without rerunning Setup; init loads the native platform reference itself when the platform it recorded is `ios`, `android`, or `adaptive`.