# Welcome CSS @apply Refactor

## Purpose

Refactor `welcome.css` and `welcome.blade.php` to replace all raw CSS values with Tailwind `@apply` directives and move inline Tailwind classes from the Blade template into named CSS classes, improving maintainability and consistency with Tailwind-first styling conventions.
